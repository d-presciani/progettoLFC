...
\item Launch4j, tool utilizzato per incapsulare l'eseguibile di tipo jar in un eseguibile di tipo exe, permettendo alcune personalizzazioni aggiuntive.
\end{itemize}
...
\If{la parte destra di $reg$ non è vuota}
\If{$reg$ non è contenuta in $regole$}
...
\If{$temp$ non è vuota}
\For{String $ch$ : $temp$}
\If{$inizi$ non contiene $ch$}
\State aggiungi $ch$ ad $inizi$
\EndIf
\EndFor
\EndIf
\If{l'elemento $i$ della parte destra di $reg$ è non è annullabile}
\State $finito$ $\leftarrow$ $true$
\EndIf
\State $i++$
\doWhile{not $finito$ $\&\&$ $i$ minore della dimensione della parte destra di $reg$}
\EndIf
\EndIf
\EndFor
\State\Return inizi
...
\par
\pagebreak
\subsubsubsection{aggiungiCompletamento}
\begin{algorithm}[H]
\caption{$nuovaRdp$: nuova regola di produzione, $regolaPadre$: regola di produzione padre)}
\label{aggiungiCompletamento}
\begin{algorithmic}[1]
\State duplico la regola di produzione $nuovaRdp$ in $regolaTemp$
\If{indice del puntino di $regolaPadre$ $+ 1 <$ dimensione della parte destra di $regolaPadre$}
\State $i \leftarrow 0$
\Do
\For{$String$ $seg :$ inizi dei caratteri di indice puntino $+ i + 1$ della parte destra di $regolaPadre$}
\If{seguiti di $regolaTemp$ non contengono $seg$}
\State aggiungo $seg$ ai seguiti di $regolaTemp$
\EndIf
\EndFor
\State $i++$
\doWhile{carattere di indice puntino $+ i$ della parte destra di $regolaPadre$ è annullabile \textbf{and} indice puntino $+ i + 1 <$ dimensione della parte destra di $regolaPadre$}
\State $verificaSegAnn \leftarrow true$
\For{$j =$ indice puntino $+ 1$ \textbf{to} $j <$ dimensione della parte destra di $regolaPadre$}
\State $verificaSegAnn = verificaSegAnn$ \&\& carattere di indice $j$ della parte destra di $regolaPadre$ è annullabile
\EndFor
\If{$regolaPadre$ è annullabile \textbf{||} dimensione della parte destra di $regolaPadre$ uguale a indice puntino $+ 1$ \textbf{||} $verificaSegAnn == true$}
\For{$String$ $seg$ : seguiti di $regolaPadre$}
\If{seguiti di $regolaTemp$ non contengono $seg$}
\State aggiungo $seg$ ai seguiti di $regolaTemp$
\EndIf
\EndFor
...
\EndFor
\EndIf
\State aggiungo $regolaTemp$ a $regoleCompletamenti$ \Comment Una volta aggiunti i seguiti, aggiungo la nuova regola alle regole di completamento
\end{algorithmic}
\end{algorithm}
